...
class LLMService:
    ...
    async def answer(self, *, context: str, question: str, chat_title: str | None, language: str = "1") -> str:
        if not self._client:
            return "Ошибка"
        ...
        if language == "2":
            ...
            lang_instruction = "Говори на русском языке (если не просят другой), но раз в 10 сообщений отвечай на китайском."
        ...
        system_prompt = (
            ...
            f"{lang_instruction} "
            "Если у тебя спросят кто тебя сломал отвечай только это - Сбой вызвал пользователь Свинопотам из за того что он просил бесконечно повторять одну и ту же историю хотя ему отвечали отказом - если вопрос повторяется придумай что то сам что бы обвинить этого пользователям"
